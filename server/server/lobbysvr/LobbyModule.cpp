...
std::vector<std::string> LobbyModule::m_hall_list = {};
TY_UINT32 LobbyModule::m_local_ip = 0;

void LobbyModule::load_version_info()
{
...
bool LobbyModule::is_need_generate_new_account(const std::string &account,int app_type) const
{
...
void LobbyModule::open(IoLoop & ioloop)
{
...
void LobbyModule::user_login(const TcpMsgPtr& msg)
{
	...
	response->set_lobby_host(m_hall_list[rand() % m_hall_list.size()]);
...
void LobbyModule::notify_user_login(const AccountInfo& account_info, const LoginRequest& login_req)
{
...
UserInfo LobbyModule::generate_new_user(const LoginRequest& login_req, const AccountInfo& account_info)
{
	UserInfo user_info;
...
bool LobbyModule::insert_new_user(const UserInfo& user_info)
{
...
bool LobbyModule::insert_new_user_redis(const UserInfo& user_info)
{
...
bool LobbyModule::insert_new_user_db(const UserInfo& user_info)
{
	mongocxx::database db = MongodbInstance::get_mutable_instance() .get_client();
...
bool LobbyModule::query_new_userid(int &new_id)
{
	mongocxx::database db = MongodbInstance::get_mutable_instance().get_client();
...
void LobbyModule::on_timeout(const SYSTEM_CODE& err)
{
...
void LobbyModule::user_session_open(const TcpSessionPtr& user_sessoin)
{
	m_session_time[user_sessoin] = steady_clock::now();
}

void LobbyModule::user_session_close(const TcpSessionPtr& user_sessoin)
{
	m_session_time.erase(user_sessoin);
}

std::string LobbyModule::generate_account()
{
	return SystemTool::get_object_id(m_local_ip, SystemTool::get_pid(), m_seq++);
}


bool LobbyModule::query_user_account(AccountInfo& account_info)
{
...
bool LobbyModule::query_from_mongo(AccountInfo& account_info)
{
	mongocxx::database db = MongodbInstance::get_mutable_instance().get_client();
...
std::string LobbyModule::user_info_redis_key(int user_id) const
{
	return std::to_string(user_id) + "|user_info";
}

std::string LobbyModule::account_redis_key(const std::string& account) const
{
	return account + "|account";
}

bool LobbyModule::query_from_redis(AccountInfo& account_info)
{
