...
void PlayerManager::on_session_close(const TcpMsgPtr &msg)
{
	...
	auto iter = m_online_user.find(msg->session_ptr().get());
...
{
	TRACE_FUNCATION();
	const UserTokenLoginRequest &request = msg->message<UserTokenLoginRequest>();
	...
	if (!check_user_token(request.usertoken(), jv))
	{
		...
		msg->send_failed_reason(ERROR_CHECK_GAME_TOKEN_FAILED);
		FULL_MSG(head, ShutDownSessionRequest, pbmsg, sendmsg);
		head->set_cmdtype(CMD_SHUTDOWN_SESSION);
		msg->session_ptr()->send_message(sendmsg);
		return false;
	}

	int user_id = jv["user_id"].int_value();
	INFO_LOG << "user(" << user_id << ") login from host(" << (msg->session_ptr()->get_addr()) 
		<< ") session(" << msg->session_ptr().get() << ")";

	//send_login_record(user_id, USER_LOGIN, msg.get_peer_addr());
	tell_user_login_game(user_id);
	msg->send_failed_reason(ERROR_SUCCESS);
	add_user_session(user_id, msg->session_ptr());
	return true;
}
...
void PlayerManager::add_user_session(int user_id, const TcpSessionPtr& session)
{
    ...
    {
		old_session = iter->second->session_ptr();
...
TcpSessionPtr PlayerManager::recv_session(int user_id)
{
    ...
    if (iter != m_login_user.end())
    {
		session = iter->second->session_ptr();
    }
    return session;
}
...
void PlayerManager::destory_player(const PLAYER_PTR &player)
{
    ...
    if (iter != m_login_user.end())
    {
        ...
        m_online_user.erase(iter->second->session_ptr().get());
...
bool PlayerManager::check_player_session(int user_id, const TcpSessionPtr& session)
{
	...
	auto iter = m_login_user.find(user_id);
	if (iter != m_login_user.end() && iter->second->session_ptr() == session)
	{
		check_result = true;
	}
...
{
	const KickOutUserRequest& request = msg->message<KickOutUserRequest>();
...
void PlayerManager::send_player_login_elsewhere(int user_id, const TcpSessionPtr& session, char reason,std::string description)
{
	WARN_LOG << "user(" << (user_id) << ") LOGIN_ELSEWHERE old sesssion("
		<< session.get() << ")";
	FULL_MSG(head, PlayerLoginElsewhereRequest, pbmsg, request);
...
void PlayerManager::shutdown_user_session(const TcpSessionPtr& session)
{
	...
	FULL_MSG(head, ShutDownSessionRequest, pbreq, close_req);
	head->set_cmdtype(CMD_SHUTDOWN_SESSION);
	close_req->set_session(session);
	session->send_message(close_req);
}
