...
public:
	...
	void to_msg_card(GG_PokerCard* card) const;
	void parse_msg_card(const GG_PokerCard&);
...
void CopyCardToMsg(const T& handCard,
	google::protobuf::RepeatedPtrField<GG_PokerCard>* pbCard)
{
	for (typename T::const_iterator it = handCard.begin(); it != handCard.end(); ++it)
	{
		GG_PokerCard *oneCard = pbCard->Add();
